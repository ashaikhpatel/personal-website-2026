...
const artifactsList = [
  ...
  { id: 3, title: "🫶🏼 ✌🏼 🤌🏼", src: '/Meher&Shradha.jpeg', category: "Art", color: "border-[#800000]" },
...
function OrnateFrame({ item, onClick }: { item: typeof artifactsList[0]; onClick: () => void }) {
  return (
    <div
      onClick={onClick}
      className={`cursor-pointer transition-all duration-700 relative group flex items-center
      ...
      <div className="relative w-full aspect-[160/280] flex items-center justify-center overflow-hidden shadow-sm bg-[#FDF5E6] rounded-sm">
        ...
        {item.src && (
          <div className="absolute inset-2 z-0 flex items-center justify-center p-5">
            <div className="relative w-[68%] h-[50%] overflow-hidden">
                <Image 
                  src={item.src} 
                  alt={item.title}
                  fill
...
export default function ArtifactsPage() {
  ...
  return (
    <main className="min-h-screen bg-[#FFF9F0] p-6 md:p-10 font-serif text-[#6B4E31] overflow-x-hidden">
      <div className="scrapbook-bg fixed inset-0 z-0 pointer-events-none opacity-30"/>
      
      <div className="max-w-6xl mx-auto relative z-10">
        <header className="text-center mb-12 md:mb-16 space-y-2">
     ...
     {selectedItem && (
      <div className="fixed inset-0 bg-[#2C1A0E]/95 backdrop-blur-sm z-50 flex items-center justify-center p-4 md:p-10 animate-in fade-in duration-500">
        <div 
          className={`relative bg-[#FDF5E6] h-fit w-fit max-w-[90vw] max-h-[85vh] 
                    border-[8px] md:border-[12px] ${selectedItem.color} shadow-2xl overflow-visible 
                    animate-in zoom-in duration-500 flex flex-col`}
        >
          <div className="relative flex-1 bg-white overflow-hidden">
            <img 
              src={selectedItem.src} 
              alt={selectedItem.title} 
              className="block w-auto h-auto max-w-full max-h-[70vh] object-contain transition-opacity duration-700"
              loading="eager"
            />
          </div>

          <div className="bg-[#FDF5E6] p-4 text-center border-t border-[#9E616A]/10">
            <h2 className="text-lg md:text-xl text-[#556B2F] italic font-medium">
              {selectedItem.title}
            </h2>
          </div>
          ...
          <button
            ...
            className="absolute -top-4 -right-4 md:-top-5 md:-right-5 w-10 h-10 md:w-12 md:h-12 rounded-full bg-[#9E616A] text-white text-2xl md:text-3xl 
